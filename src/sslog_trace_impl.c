                              ...
                              int *num_methods )
{
    ...
    if (    ( info_A = strstr( newline, "< " ) ) != NULL
        ...
        sprintf( info_B, "%c", '\0' );
#if TAU	
	/* OK, now we simply ignore whatever is in it, and treat
	   everything as %s */
        strncpy( label, "%s", MAX_LABEL_LEN );
#else
...
*/
    ...
    *num_bytes = 0;
    ...
    if (    ( info_A = strstr( newline, "< " ) ) != NULL
        ...
        sprintf( info_B, "%c", '\0' );
#if TAU
	/* here the goal is to have a char array whose first
	   two bytes describes the length (in short int) 
	   the string that follows. It must be in big endian. */
        // sscanf( info_A, type->label, xinfovals + 2);
	xinfovals = (char *)malloc(strlen(info_A) + 3);
	strcpy( xinfovals + 2, info_A);
#else
...
#endif
#if TAU
        *num_bytes = strlen(info_A) + 3;
	*((short*)xinfovals) = *num_bytes - 2;
#if ! defined( WORDS_BIGENDIAN )
	char x = xinfovals[0];
	xinfovals[0] = xinfovals[1];
	xinfovals[1] = x;
#endif
#else
        *num_bytes = 8;
...
#if TAU
        prime->info      = xinfovals;
#else
        prime->info      = (char *) malloc( prime->num_info * sizeof(char) );
        memcpy( prime->info, xinfovals, prime->num_info );
#if ! defined( WORDS_BIGENDIAN )
